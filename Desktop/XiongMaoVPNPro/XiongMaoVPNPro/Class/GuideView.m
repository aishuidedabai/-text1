...
//    btnMaskRect.size = CGSizeMake(floor(100), floor(200));
//    btnMaskRect.origin = CGPointMake(floor(20), floor(200));
//    _btnMaskView.frame = btnMaskRect;
    
    _btnMaskView.frame=CGRectMake(([UIScreen mainScreen].bounds.size.width-260)/2, [UIScreen mainScreen].bounds.size.height/3.2, 260, 200);
    ...
    _okBtn.bottom = _btnMaskView.top + _btnMaskView.frame.size.height+100;
}
...
- (void)dismiss {
    //创建通知
    NSNotification *notification =[NSNotification notificationWithName:@"anzhuang" object:nil userInfo:nil];
    //通过通知中心发送通知
...
- (UIImageView *)btnMaskView {
    if (!_btnMaskView) {
        UIImage *image = [UIImage imageNamed:@"konw_box_2"];
       // image = [image maskImage:[[UIColor whiteColor] colorWithAlphaComponent:1]];
