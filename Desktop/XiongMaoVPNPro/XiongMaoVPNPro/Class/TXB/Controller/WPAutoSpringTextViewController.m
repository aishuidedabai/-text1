...
- (CGFloat)shouldScrollWithKeyboardHeight:(CGFloat)keyboardHeight{
    ...
    if([responder isKindOfClass:[UITextView class]] || [responder isKindOfClass:[UITextField class]]){
        ...
        //键盘高度
        NSLog(@"键盘高度shouldScrollWithKeyboardHeight -->keyboradHeight %@, keyboradBottom %@, viewY %@, bottom %@", @(keyboardHeight), @(SCREEN_HEIGHT - keyboardHeight), @(y), @(bottom));
        if(bottom > SCREEN_HEIGHT - keyboardHeight){
            return bottom - (SCREEN_HEIGHT - keyboardHeight)+50;
        }
    }
    return 0;
}
