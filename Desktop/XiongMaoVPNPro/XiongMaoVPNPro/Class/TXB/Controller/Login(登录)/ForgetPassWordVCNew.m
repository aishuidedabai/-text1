...
-(void)registYanZheng
{
    if ([_phoneTextField.text isEqualToString:@""]||_phoneTextField.text == nil) {
        ...
        NSDictionary *parameters = @{@"sms_type":@"SMS_FORGET_PASSWORD",@"phone":_phoneTextField.text};
...
-(void)sureData
{
        ...
        NSDictionary *parameters = @{@"phone":_phoneTextField.text,@"password":_passwordText.text,@"code":_yanzhengTextF.text};
        [EncryptAndEcode httpUrl:@"http://api.xiongmaoapi.com/api/xiongmao/forgot_my_password/" AndPostparameters:parameters block:^(NSDictionary *responseObject) {
    
              NSLog(@"----------修改密码------%@",responseObject);
             NSString *str=[NSString stringWithFormat:@"%@",[responseObject valueForKey:@"success"]];
            if([str isEqualToString:@"0"])
            {
                LXAlertView *alert1=[[LXAlertView alloc] initWithTitle:@"提示" message:@"修改失败" cancelBtnTitle:@"取消" otherBtnTitle:@"确定" clickIndexBlock:^(NSInteger clickIndex) {
                ...
                //存进去沙盒
                NSDictionary *dd=@{SHInvokerUserInfoPhoneNo:_phoneTextField.text};
                [SHInvoker saveUserInfo:dd];
                
                LXAlertView *alert1=[[LXAlertView alloc] initWithTitle:@"提示" message:@"修改成功" cancelBtnTitle:@"取消" otherBtnTitle:@"确定" clickIndexBlock:^(NSInteger clickIndex) {
                    [self returnLoginBtn];
                }];
                alert1.animationStyle=LXASAnimationLeftShake;
                [alert1 showLXAlertView];
